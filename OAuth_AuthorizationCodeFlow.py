...
def request_tokens(**kwargs):
	...
	authServerId = kwargs['authServerId']
	...
	if "sessionToken" in response:
		...
		authorizeUrl ="https://" + orgUrl + "/oauth2/" + authServerId + "/v1/authorize?sessionToken="+sessionToken+"&client_id="+clientId+"&scope=" + scopes + "&response_type=code&response_mode=fragment&nonce=staticNonce&redirect_uri="+redirectUri+"&state=staticState"
		...
		if code.find("error") > -1:
			...
			getTokenUrl = "https://" + orgUrl + "/oauth2/" + authServerId + "/v1/token?grant_type=authorization_code&code=" + code + "&redirect_uri=" + redirectUri

			response = post_request(getTokenUrl, headers, None)

			if "error" in response or "errorCode" in response:

				print "Error occurred while executing POST /token request\n"
				print response
...
if __name__ == "__main__":
	...
	parser.add_argument("-authServerId",
					help="Authorizaiton Server Id from Security -> API -> Authrization Servers",
					required="True",
					default=":authorizationServerId")
	...
	request_tokens(
			...
			authServerId = arguments.authServerId,
